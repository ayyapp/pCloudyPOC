...
public class test extends Abstract {
	...
	public static void AfterScenario() throws MalformedURLException, IOException, APIException{
		...
		Gurock.TestRail.APIClient client = new Gurock.TestRail.APIClient(â€œhttps://yoursite.testrail.com/â€�);

		client.User = â€œuser@domain.comâ€�; //user e-mail

		client.Password = â€œSpecFlowâ€�; //user password

		Dictionary<string, object> testResult = new Dictionary<string, object>();

		if(null != ScenarioContext.Current.TestError){

		testResult[â€œstatus_idâ€�] = â€œ5â€�; //failed;

		testResult[â€œcommentâ€�] = ScenarioContext.Current.TestError.ToString();
		}

		else

		{
			testResult[â€œstatus_idâ€�] = â€œ1â€�; //passed
		}


		client.SendPost(â€œadd_result/1â€�, testResult); //hardcoded test id.
		}
